...
int main(int argc, char **argv) {
    ...
    for(size_t i = 0; i < NFILES; ++i) {
        size_t len = printf("==== LIST %zu (%zu INSTANCES) ====\n", i + 1, arrlenu(lists[i]));
    ...
    size_t count = 0;
    for(size_t i = 0; i < arrlenu(lists[1]); ++i) if(shgeti(dict, lists[1][i]) == -1) printf("%zu >> %s\n", ++count, lists[1][i]);
    printf("=================NOT MATCHING LIST END=================\n");
    printf("-> Number of users that didn't match = %zu\n", count);
...
void list_print(char **list) {
    for(size_t i = 0; i < arrlenu(list); ++i) printf("%zu >> %s\n", i + 1, list[i]);
}
